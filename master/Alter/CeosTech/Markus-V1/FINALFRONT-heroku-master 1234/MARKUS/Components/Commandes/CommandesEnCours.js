...
async function updateCommandeLivre(formula, id) {
  const response = await fetch("https://markus-app.herokuapp.com/commande/update-commande",
   {
       ...
       body: JSON.stringify({ id, est_livre: formula, }),
     })
...
class CommandesEnCours extends React.Component {
        ...
        <FlatList
          ...
          renderItem={({item}) => {
            if (item.est_vue && !item.est_livre) {
              return (
                    <CommandesEnCoursItem update={updateCommandeLivre} updateState={() => this.setState({update: this.state.update+1})} item={item} />
                )
            } else {
              return null;
            }
              // return (
              //     <CommandesEnCoursItem item={item}/>
              // )
          }}
        />
      }
