...
const Activite = () => {
  ...
  const [nbAdherents, setNbAdherents] = useState(0);
  const [nbDonnateurs, setNbDonnateurs] = useState(0);
  const aujourdhui = new Date().toLocaleDateString(); //// 11/03/2021

  const fetchNbAdherent = async () => {
    await axios
      .get(URL + "adhesions/")
      .then((res) => setNbAdherents(res.data.length))
      .catch((err) => console.log(err));
  };
  const fetchNbDonnateurs = async () => {
    await axios
      .get(URL + "dons/")
      .then((res) => setNbDonnateurs(res.data.length))
      .catch((err) => console.log(err));
  };
  const get_suivi_jour = async (debut, fin, setState) => {
    const { data } = await axios.post(URL + "dons/stripe/transactions", {
      date: {
        gte: debut,
        lte: fin,
      },
    ...
    <div className="suivi_activite admin__container">
      <div className="suivi_activite__jour">
        <div className="suivi_activite__jour__CA">
          <p className="suivi_activite__jour__CA-heading">Adhérent</p>
          <div className="suivi_activite__jour__CA__card">
            <h1>{nbAdherents}</h1>
          </div>
        </div>

        <div className="suivi_activite__jour__operations">
          <p className="suivi_activite__jour__operations-heading">Donateur</p>
          <div className="suivi_activite__jour__operations__card">
            <h1>{nbDonnateurs}</h1>
          </div>
        </div>
      </div>
      ...
      <div className="suivi_activite__jour">
        <div className="suivi_activite__jour__CA">
          ...
          <div className="suivi_activite__jour__CA__card">
            <h1>{splitPrix(aujourdhui_ca ? aujourdhui_ca.amount : 0)}</h1>
          </div>
        </div>
        <div className="suivi_activite__jour__operations">
          <p className="suivi_activite__jour__operations-heading">
            Total opérations du jour
          </p>
          <p className="suivi_activite__jour__operations-date">{aujourdhui}</p>

          <div className="suivi_activite__jour__operations__card">
            <h1>{aujourdhui_ca ? aujourdhui_ca.nb_dons : 0}</h1>
          </div>
        </div>
      </div>
      ...
      <div className="suivi_activite__jour_exacte">
        ...
        <div className="suivi_activite__jour">
          <div className="suivi_activite__jour__CA">
            ...
            <div className="suivi_activite__jour__CA__card">
              <h1>{splitPrix(calendar_ca ? calendar_ca.amount : 0)}</h1>
            </div>
          </div>
          <div className="suivi_activite__jour__operations">
            <p className="suivi_activite__jour__operations-heading">
            ...
            <p className="suivi_activite__jour__operations-date">{calendar}</p>

            <div className="suivi_activite__jour__operations__card">
              <h1>{calendar_ca ? calendar_ca.nb_dons : 0}</h1>
            </div>
          </div>
        </div>
      </div>
